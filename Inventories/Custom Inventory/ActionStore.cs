...
namespace Musialkov.Inventories
{
    public class ActionStore : MonoBehaviour, ISaveable
    {
        ...
        public void AddAction(InventoryItem item, int index, int number, bool isRestoring)
        {
            if (dockedItems.ContainsKey(index))
            {  
                if (object.ReferenceEquals(item, dockedItems[index].item) && !isRestoring)
                {
        ...
        void ISaveable.RestoreState(object state)
        {
            ...
            {
                AddAction(InventoryItem.GetFromID(pair.Value.itemID), pair.Key, pair.Value.number, true);
            }
        }
    }
}